...
func (e *EbpfEnv) executeVM(ctx context.Context, beaconFlatbuffer []byte, job *cppb.RACJob, code []byte) (*cppb.JobCompleteNotify, error) {
	...
	for i := 0; i < int(ret.result_len); i++ {
		...
		selection = append(selection, &cppb.BeaconAndEgressIntf{Selected: job.RowIds[beacon_res.beacon_id], EgressIntfs: intfs})
	}
	...
	fmt.Printf("EBPF TIME; %d, %d, %d, %d\n",
		time.Since(totalExec),
		timeTillSubmitNoLoad.Nanoseconds(),
		execTime.Sub(prepareMemTime).Nanoseconds(),
		totalExec.Sub(execTime).Nanoseconds())
	if e.Static {
