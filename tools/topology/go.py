...
class GoGenerator(object):
    ...
    def build_rac_conf(self, topo_id, ia, base, name, v, ctrl_addr):
        ...
        raw_entry = {
            ...
            'rac': {'ctrl_addr' : ctrl_addr, 'addr': v.get("addr", ""), 'static': v.get("static", False), 'static_algorithm': {'file': v.get("static_algorithm", "")}}
        }
        return raw_entry
    ...
    def generate_irec(self, topo_id):
        ...
        for alg in algs:
            if 'originate' in alg and 'file' in alg and 'id' in alg:
                sanitized_list.append({'originate': alg['originate'], 'file': alg['file'], 'id': alg['id'], 'fallback': alg['fallback'] if 'fallback' in alg else False})
