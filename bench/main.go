...
func MockBeacon(ases []IfInfo, id uint16, inIfId uint16, infoTS int64) (beacon.Beacon, *cppb.IRECPathSegment, []byte, []byte) {
	...
	if err != nil {
		...
		return beacon.Beacon{}, &cppb.IRECPathSegment{}, nil, []byte{}
	}
...
func main() {
	...
	if os.Args[3] == "rawthr" {
		...
		for i := 0; i < amountOfBeacons; i++ {
			...
			var excerpt *cppb.IRECPathSegment
			var fbs []byte
			switch r1.Intn(5) {
			case 0:
	...
	for i := 0; i < amountOfBeacons; i++ {
		...
		igdb.AddAlgorithm(context.Background(), []byte{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09}, fileBinary)
	...
	if os.Args[3] == "latency" || os.Args[3] == "bw" {
		...
		env := BenchEnv{}
		if os.Args[3] == "bw" {
			env.RegisterServers(&EgressDummy{}, &is, true)
		} else {
...
type EgressDummy struct {
}

func (e *EgressDummy) PullBasedCallback(ctx context.Context, incomingBeacon *cppb.IncomingBeacon) (*cppb.IncomingBeaconResponse, error) {
	//TODO implement me
	panic("implement me")
}

func (e *EgressDummy) RequestPullBasedOrigination(ctx context.Context, request *cppb.PullPathsRequest) (*cppb.PropagationRequestResponse, error) {
	//TODO implement me
	panic("implement me")
}

func (e *EgressDummy) RequestPropagation(ctx context.Context, request *cppb.PropagationRequest) (*cppb.PropagationRequestResponse, error) {
