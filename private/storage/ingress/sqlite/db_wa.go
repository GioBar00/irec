...
// DYNAMIC RACS
func (e *executor) GetBeaconJob(ctx context.Context, ignoreIntfGroup bool, fetchExpirationTime time.Time) ([]*cppb.IRECBeacon, []byte, []int64, error) {
	e.Lock()
	defer e.Unlock()
	//
	//INSERT INTO Retrieved(RowID)
	//SELECT DISTINCT b2.RowID from Beacons b2,
	//	(SELECT DISTINCT b.StartIsd, b.StartAs, b.StartIntfGroup, b.AlgorithmHash, b.AlgorithmId FROM Beacons b, Algorithm a WHERE FetchStatus = 0 AND b.AlgorithmHash = a.AlgorithmHash ORDER BY RANDOM() LIMIT 1) selected WHERE b2.StartIsd = selected.StartIsd AND b2.StartAs = selected.StartAs`
	//if !ignoreIntfGroup {
	//	query = query + ` AND b2.StartIntfGroup = selected.StartIntfGroup`
	//}
	//query = query +
	//	` AND b2.AlgorithmHash = selected.AlgorithmHash AND b2.AlgorithmId = selected.AlgorithmId AND b2.FetchStatus = 0;
	//UPDATE Beacons SET FetchStatus = 1 FROM Retrieved r WHERE Beacons.RowID = r.RowId;
	//
	//SELECT DISTINCT LastUpdated, Beacon, InIntfID, Usage, Flatbuffer FROM Beacons b, Retrieved r
	//WHERE b.RowID = r.RowId;
	tx, err := e.db.(*sql.DB).BeginTx(ctx, nil)
	if err != nil {
		return nil, []byte{}, []int64{}, serrors.WrapStr("looking up beacons", err)
	}
...
LIMIT 1)
	...
	rows, err := tx.QueryContext(ctx, "SELECT DISTINCT b.RowID, LastUpdated, Beacon, InIntfID, Usage, AlgorithmHash FROM Beacons b, Retrieved r  WHERE b.RowID = r.RowId ORDER BY b.HopsLength ASC, b.LastUpdated DESC;")
	if err != nil {
	...
	for rows.Next() {
		...
		seg, err := beacon.UnpackBeaconIRECExcerpt(rawBeacon)
...
// DEPRECATED
func (e *executor) GetAndMarkBeacons(ctx context.Context, maximum uint32, algHash []byte, algId uint32, originAS addr.IA, originIntfGroup uint32, ignoreIntfGroup bool, fetchStatus uint32) ([]*cppb.IRECBeacon, error) {
...
// USED BY STATIC RACS
func (e *executor) GetBeacons(ctx context.Context, opts *beacon2.QueryOptions) ([]*cppb.IRECBeacon, []int64, error) {
	e.RLock()
	defer e.RUnlock()
	...
	if err != nil {
		return nil, []int64{}, serrors.WrapStr("looking up beacons", err, "query", stmt)
	}
	defer rows.Close()
	var res []*cppb.IRECBeacon
	var rowIds []int64
	...
	for rows.Next() {
		...
		err = rows.Scan(&RowID, &lastUpdated, &rawBeacon, &InIntfID, &usage)
		if err != nil {
			return nil, []int64{}, serrors.WrapStr("reading row", err)
		}
		rowIds = append(rowIds, RowID)
		seg, err := beacon.UnpackBeaconIRECExcerpt(rawBeacon)

		if err != nil {
			return nil, []int64{}, serrors.WrapStr("parsing beacon", err)
		}
		res = append(res, &cppb.IRECBeacon{
			PathSeg: seg,
			InIfId:  uint32(InIntfID),
