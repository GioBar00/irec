...
// DYNAMIC RACS
func (e *executor) GetBeaconJob(ctx context.Context, ignoreIntfGroup bool, fetchExpirationTime time.Time) ([][]byte, []byte, []int64, error) {
	e.Lock()
	defer e.Unlock()
...
// DEPRECATED
func (e *executor) GetAndMarkBeacons(ctx context.Context, maximum uint32, algHash []byte, algId uint32, originAS addr.IA, originIntfGroup uint32, ignoreIntfGroup bool, fetchStatus uint32) ([][]byte, error) {
	...
	if err != nil {
		return [][]byte{}, serrors.WrapStr("looking up beacons", err, "query", selStmt)
	}
	defer rows.Close()
	var fbs [][]byte
	var count uint64 = 0
	for rows.Next() {
		...
		var usage beacon.Usage
		var flatbuffer sql.RawBytes
		err = rows.Scan(&RowID, &lastUpdated, &InIntfID, &usage, &flatbuffer)
		if err != nil {
			return [][]byte{}, serrors.WrapStr("reading row", err)
		}
		fbs = append(fbs, flatbuffer)
		count += 1
	}
	...
	if err != nil {
		...
		return [][]byte{}, db.NewWriteError("updating fetchStatuses", err)
	}
	return fbs, nil
...
// USED BY STATIC RACS
func (e *executor) GetBeacons(ctx context.Context, opts *beacon2.QueryOptions) ([][]byte, []int64, error) {
	e.RLock()
	defer e.RUnlock()
	...
	if err != nil {
		return [][]byte{}, []int64{}, serrors.WrapStr("looking up beacons", err, "query", stmt)
	}
	defer rows.Close()
	var fbs [][]byte
	var rowIds []int64
	for rows.Next() {
		...
		err = rows.Scan(&RowID, &lastUpdated, &InIntfID, &usage, &flatbuffer)
		if err != nil {
			return [][]byte{}, []int64{}, serrors.WrapStr("reading row", err)
		}
		rowIds = append(rowIds, RowID)
		fbs = append(fbs, flatbuffer)
	}
	...
	if err := rows.Err(); err != nil {
		log.Error("err when selecting beacons", "err", err)
		return [][]byte{}, []int64{}, err
	}
	return fbs, rowIds, nil
}
func (e *executor) buildQuery(ctx context.Context, opts *beacon2.QueryOptions) (string, []interface{}) {
	var args []interface{}
	query := "SELECT DISTINCT b.RowID, b.LastUpdated, b.InIntfID, b.Usage, b.Flatbuffer FROM Beacons b"
...
func insertNewBeacon(
	...
	//inst := `
	//INSERT INTO Beacons (SegID, FullID, StartIsd, StartAs, StartIntfGroup, AlgorithmHash, AlgorithmId, InIntfID, HopsLength, InfoTime,
	//	ExpirationTime, LastUpdated, Usage, Flatbuffer, FetchStatus, FetchStatusExpirationTime, PullBased, PullBasedMinBeacons, PullBasedPeriod, PullBasedHyperPeriod, PullBasedTargetAs, PullBasedTargetIsd, Beacon)
	//VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, ?, ?,?, ?, ?, ?, NULL)
	//`
