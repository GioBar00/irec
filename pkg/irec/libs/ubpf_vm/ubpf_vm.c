...
int
ubpf_exec(const struct ubpf_vm* vm, struct ubpf_ctx* calling_ctx, void* mem, size_t mem_len, uint64_t* bpf_return_value)
{
    uint16_t pc = 0;
    ...
    do {                                                                                                         \
        ...
        case EBPF_OP_EXIT:
            *bpf_return_value = reg[0];
            return 0;
        case EBPF_OP_CALL:
            // Unlike xBGP, we pass the vm_context directly, instead of rewriting instructions to include a pointer
            // to the context. This prevents some overhead.
            reg[0] = vm->ext_funcs[inst.imm](calling_ctx, reg[1], reg[2], reg[3], reg[4], reg[5]);
...
validate(const struct ubpf_vm* vm, const struct ebpf_inst* insts, uint32_t num_insts, char** errmsg)
{
    ...
    // Chcek if there even is an exit instr, to prevent endless fn
    ...
    for (i = 0; i < num_insts; i++) {
        ...
        case EBPF_OP_CALL:
            ...
            if (!vm->ext_funcs[inst.imm]) {
                *errmsg = ubpf_error("call to nonexistent function %u at PC %d. If you see this, make sure your binary does not define any functions. If it does, make sure to inline these functions using ```static inline __attribute__((always_inline))```, as is mandatory per eBPF standard.", inst.imm, i);
//                printf("error msg, instr: %d, looking for fn %d\n", i, inst.imm);
                return false;
            }
    ...
    void* stack)
{
    if (!vm->bounds_check_enabled)
        return true;
    if (addr >= stack && ((char*)addr + size) <= ((char*)stack + UBPF_STACK_SIZE)) {
        /* Stack access */
        return true;
    } else if (vm->extra_mem_size != 0 && // compare only if this VM contains extra memory
        ((uint8_t *) addr >= vm->extra_mem_start && ((char*)addr + size) <= ((char*) vm->extra_mem_start + vm->extra_mem_size))
            ) {
        /* Extra memory access */
        return true;
