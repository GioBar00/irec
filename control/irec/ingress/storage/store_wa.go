...
type IngressStore interface {
	GetBeacons(ctx context.Context, req *cppb.BeaconQuery) ([]*cppb.IRECBeacon, []int64, error)
	GetBeaconJob(ctx context.Context, ignoreIntfGroup bool) ([]*cppb.IRECBeacon, []byte, []int64, error)
...
type DB interface {
	...
	MarkBeacons(ctx context.Context, ids []int64) error
	GetBeaconJob(ctx context.Context, ignoreIntfGroup bool, fetchExpirationTime time.Time) ([]*cppb.IRECBeacon, []byte, []int64, error)
	GetBeacons(ctx context.Context, opts *beacon2.QueryOptions) ([]*cppb.IRECBeacon, []int64, error)
	GetBeaconsByRowIDs(ctx context.Context, ids []int64) ([]*cppb.EgressBeacon, error)
	GetBeaconByRowID(ctx context.Context, id int64) (*cppb.EgressBeacon, error)
...
func (s *baseStore) GetBeaconJob(ctx context.Context, ignoreIntfGroup bool) ([]*cppb.IRECBeacon, []byte, []int64, error) {
	return s.db.GetBeaconJob(ctx, ignoreIntfGroup, time.Now().Add(time.Second*30))
}

func (s *baseStore) GetBeacons(ctx context.Context, req *cppb.BeaconQuery) ([]*cppb.IRECBeacon, []int64, error) {
	...
	beacons, rowIds, err := s.db.GetBeacons(ctx, &beacon2.QueryOptions{
		Maximum:      req.Maximum,
	...
	if err != nil {
		return nil, []int64{}, serrors.WrapStr("retrieving beacons failed", err)
	}

	return beacons, rowIds, nil
}
