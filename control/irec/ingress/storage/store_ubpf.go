...
type IngressStore interface {
	GetBeacons(ctx context.Context, req *cppb.BeaconQuery) ([][]byte, []int64, error)
	GetBeaconJob(ctx context.Context, ignoreIntfGroup bool) ([][]byte, []byte, []int64, error)
	...
	//DEPRECATED;
	GetAndMarkBeacons(ctx context.Context, req *cppb.RACBeaconRequest) ([][]byte, error)
...
type DB interface {
	GetAndMarkBeacons(ctx context.Context, maximum uint32, algHash []byte, algID uint32, originAS addr.IA, originIntfGroup uint32, ignoreIntfGroup bool, marker uint32) ([][]byte, error)
	BeaconSources(ctx context.Context, ignoreIntfGroup bool) ([]*cppb.RACBeaconSource, error)
	InsertBeacon(ctx context.Context, b beacon.Beacon, usage beacon.Usage) (beacon.InsertStats, error)

	MarkBeacons(ctx context.Context, ids []int64) error
	GetBeaconJob(ctx context.Context, ignoreIntfGroup bool, fetchExpirationTime time.Time) ([][]byte, []byte, []int64, error)
	GetBeacons(ctx context.Context, opts *beacon2.QueryOptions) ([][]byte, []int64, error)
	GetBeaconsByRowIDs(ctx context.Context, ids []int64) ([]*cppb.EgressBeacon, error)
	GetBeaconByRowID(ctx context.Context, id int64) (*cppb.EgressBeacon, error)
...
func (s *baseStore) GetAndMarkBeacons(ctx context.Context, req *cppb.RACBeaconRequest) ([][]byte, error) {
	fbs, err := s.db.GetAndMarkBeacons(ctx, req.Maximum, req.AlgorithmHash, req.AlgorithmID, addr.IA(req.OriginAS), req.OriginIntfGroup, req.IgnoreIntfGroup, 1)
	if err != nil {
...
func (s *baseStore) GetBeaconJob(ctx context.Context, ignoreIntfGroup bool) ([][]byte, []byte, []int64, error) {
	return s.db.GetBeaconJob(ctx, ignoreIntfGroup, time.Now().Add(time.Second*30))
}

func (s *baseStore) GetBeacons(ctx context.Context, req *cppb.BeaconQuery) ([][]byte, []int64, error) {
	...
	fbs, rowIds, err := s.db.GetBeacons(ctx, &beacon2.QueryOptions{
		Maximum:      req.Maximum,
	...
	if err != nil {
		return [][]byte{}, []int64{}, serrors.WrapStr("retrieving beacons failed", err)
	}

	return fbs, rowIds, nil
}
