...
def scion_app_image_kathara(name, src, entrypoint, appdir = "/app", workdir = "/share", cmd = None, caps = None, caps_binary = None):
    pkg_tar(
        name = "%s_kathara_docker_files" % name,
        srcs = [src],
        package_dir = appdir,
        mode = "0755",
    )
    oci_image(
        name = "%s_kathara" % name,
        base = "//docker:app_base_kathara",
        tars = [":%s_kathara_docker_files" % name],
        workdir = workdir,
    ...
    copy_file(
        name = name + "_kathara.tarball",
        src = name + "_kathara.load",
        out = name + "_kathara.tar",
        visibility = ["//visibility:public"],
    )
    oci_tarball(
        name = name + "_kathara.load",
        format = "docker",
        image = name + "_kathara",
        repo_tags = ["scion/" + name + ":kathara-irec"],
        visibility = ["//visibility:public"],
    )
